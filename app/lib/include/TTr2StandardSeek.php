 ...
 */
class TTr2StandardSeek extends TWindow
{
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            $pk   = empty(TSession::getValue('standard_seek_model_key'))? constant("{$model}::PRIMARYKEY"): TSession::getValue('standard_seek_model_key');
    ...
    public function onSetup($param=NULL)
    {
        ...
        if (isset($param['hash']) AND $param['hash'] == md5($seed.$param['database'].$param['model'].$param['display_field']))
        {
            ...
            TSession::setValue('standard_seek_model_key',     ($param['model_key']  ?? null) );
    ...
    public static function onSelect($param)
    {
        ...
        //$criteria      =     ?  : (TSession::getValue('standard_seek_criteria')) ? clone TSession::getValue('standard_seek_criteria') : new TCriteria;
        
        if(isset($param['criteria']))
        {
            $criteria = (unserialize(base64_decode($param['criteria'])) instanceof TCriteria) ? clone unserialize(base64_decode($param['criteria'])) : new TCriteria;
        }
        else
        {
            if(!empty(TSession::getValue('standard_seek_criteria')))
            {
                $criteria = clone TSession::getValue('standard_seek_criteria');
            }
            else
        ...
        try
        {
            ...
            if(isset($param['model_key']))
            {
                $pk = $param['model_key'];
            }
            else
            {
                if(!empty(TSession::getValue('standard_seek_model_key')))
                {
                    $pk = TSession::getValue('standard_seek_model_key');    
                }
                else
                {
                    $pk = $model::PRIMARYKEY;
