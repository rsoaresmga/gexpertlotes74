 ...
 */
class TTr2SeekButton extends TEntry implements AdiantiWidgetInterface
{
    private $action;
    private $useOutEvent;
    private $button;
            ...
            if ($this->action)
            {
                ...
                if (is_array($callback = $this->action->getAction()))
                {
                    ...
                    if ($this->useOutEvent)
                    {
                        ...
                        if (in_array($classname, array('TTr2StandardSeek')))
                        {
                            ...
                            $ajaxAction->setParameter('model_key',     $this->action->getParameter('model_key') ? $this->action->getParameter('model_key') : NULL);
